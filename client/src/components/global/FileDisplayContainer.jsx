      ...
      {images.map((image, idx) => (
        <div key={idx} className="relative">
          <img
            ...
            alt={idx}
            className="w-16 h-16 gap-6"
          />
          <AiOutlineClose
            className="absolute -top-1 -right-2 border rounded-full text-red-400 border-red-400 text-xl bg-white cursor-pointer"
      ...
      {files.map((file, idx) => (
        <>
          {file.name.split('.')[file.name.split('.').length - 1] === 'pdf' ? (
            <div
              className="border-2 w-16 h-16 flex items-center justify-center relative"
              key={idx}
            >
              ...
              .split('.')
              [file.name.split('.').length - 1].includes('xlsx', 'xls') ? (
            <div
              className="border-2 w-16 h-16 flex items-center justify-center relative"
              key={idx}
            >
              ...
              .split('.')
              [file.name.split('.').length - 1].includes('docx', 'doc') ? (
            <div
              className="border-2 w-16 h-16 flex items-center justify-center relative"
              key={idx}
            >
              ...
              .split('.')
              [file.name.split('.').length - 1].includes('pptx', 'ppt') ? (
            <div
              className="border-2 w-16 h-16 flex items-center justify-center relative"
              key={idx}
            >

...
const Message = ({
  ...
  return (
    <div
      className={`mb-7 flex ${
        type === 'sender' ? 'items-end' : 'items-start'
      } ${type === 'sender' ? 'justify-end' : 'justify-start'} flex-col`}
    >
      {message && (
        <div
          className={`flex ${
            type === 'sender' ? 'flex-row-reverse' : undefined
          }`}
        >
          <Avatar
            src={type === 'sender' ? auth.user?.avatar : recipientAvatar}
            size="30px"
          />
          <div
            className={`max-w-[20rem] w-fit float-right break-all ${
              type === 'sender' ? 'bg-sky-400' : 'bg-gray-100'
            } p-3 rounded-md mb-2 ${
              type === 'sender' ? 'text-white' : undefined
            } ${type === 'sender' ? 'mr-3' : 'ml-3'} flex items-center gap-2`}
          >
            {message}
      ...
      {media.length > 0 && (
        ...
        >
          ...
          <div
            className={`${
              type === 'sender'
                ? `${message ? '-translate-x-[50px]' : '-translate-x-3'}`
                : `${message ? 'translate-x-[50px]' : 'translate-x-3'}`
            }`}
          >
            {media.map((item, idx) => (
              ...
              <>
                {item.split('.')[item.split('.').length - 1] === 'pdf' ? (
                  <div className="flex items-center justify-center border rounded-md border-gray-500 p-3 gap-6 mb-2">
                    ...
                    .split('.')
                    [item.split('.').length - 1].includes('docx', 'doc') ? (
                  <div className="flex items-center justify-center border rounded-md border-gray-500 p-3 gap-6 mb-2">
                    <div className="flex items-center justify-center gap-2">
                    ...
                    .split('.')
                    [item.split('.').length - 1].includes('xlsx', 'xls') ? (
                  <div className="flex items-center justify-center border rounded-md border-gray-500 p-3 gap-6 mb-2">
                    <div className="flex items-center justify-center gap-2">
                    ...
                    .split('.')
                    [item.split('.').length - 1].includes('pptx', 'ppt') ? (
                  <div className="flex items-center justify-center border rounded-md border-gray-500 p-3 gap-6 mb-2">
                    <div className="flex items-center justify-center gap-2">
      ...
      <p
        className={`${
          type === 'sender' ? '-translate-x-[50px]' : 'translate-x-[50px]'
        } text-gray-400`}
      >
        {timestamp}
      </p>
